    ...
    <div className="relative mx-auto flex min-h-full max-w-lg flex-col justify-center gap-9 p-6">
      ...
      <div className="flex flex-col items-center gap-4">
        ...
        <div className="pt-8 text-center">
          <p className="font-display text-[10px] font-extrabold tracking-[0.45em] text-gold/70">{theme.eyebrow}</p>
          <h1 className="mt-1 font-display text-[46px] font-extrabold leading-none tracking-tight">
            ラッキー<span className="text-seven [text-shadow:0_0_34px_var(--glow)]">7</span>
          </h1>
        </div>
      </div>
      ...
      <div className="w-full space-y-3">
        ...
        <div className="flex gap-2">
          <input
            className="min-w-0 flex-1 rounded-2xl border border-edge/10 bg-ink2/90 px-4 py-3.5 text-center font-display text-xl font-extrabold tracking-[0.3em] outline-none transition focus:border-gold/50 focus:ring-2 focus:ring-gold/25"
            value={code}
            maxLength={6}
            inputMode="numeric"
            pattern="[0-9]*"
          ...
          />
          <button
            className="shrink-0 rounded-2xl border border-edge/12 bg-ink3 px-6 font-bold text-cream/85 transition active:scale-95 disabled:opacity-40"
        ...
        <div className="flex items-center gap-2 pt-2">
          ...
          <Link
            to="/about"
            aria-label="設定・情報"
            className="flex min-h-[48px] shrink-0 items-center gap-1.5 rounded-2xl border border-edge/10 bg-ink3/70 px-4 text-[14px] font-bold text-cream/75 transition active:scale-[.98]"
          >
            <span aria-hidden>⚙</span>
            情報
          </Link>
